...
def take_measurement(sysi, probe_cube, probe_amplitude, channel=3, pca_modes=None, plot=False):
    ...
    for i in range(N_probes):
        ...
        sysi.add_dm(probe_amplitude * probe, channel=channel) # add positive probe
        im_pos = sysi.snap_camsci()
        sysi.add_dm(-probe_amplitude * probe, channel=channel) # remove positive probe
        sysi.add_dm(-probe_amplitude * probe, channel=channel) # add negative probe
        im_neg = sysi.snap_camsci()
        sysi.add_dm(probe_amplitude*probe, channel=channel) # remove negative probe
...
def run(sysi, 
    ...
    for i in range(num_iterations):
        ...
        diff_ims = take_measurement(sysi, probe_modes, probe_amplitude, channel=channel, plot=plot_probes)
        ...
        del_command = gain * modal_matrix.T.dot(modal_coeff).reshape(sysi.Nact,sysi.Nact)
        total_command = (1.0-leakage)*total_command + del_command
        # sysi.add_dm(del_command, channel=channel)
        sysi.set_dm(total_command, channel=channel)
